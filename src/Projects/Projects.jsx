...
const Projects = () => {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen mt-[450px]">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7 p-4 max-w-[1170px]">
        {items.map((item, index) => (
          ...
          >
            {item.title && (
              <div className="absolute w-[168px] bottom-4 left-4 bg-opacity-50 text-white text-3xl font-manrope-extrabold p-2 rounded">
                {item.title}
              </div>
            )}
          </div>
